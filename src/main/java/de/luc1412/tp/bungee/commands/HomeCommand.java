			...
			if (player.hasPermission("tp.home")) {
				if (args.length == 0) {
					...
					try {
						...
						player.sendMessage(BungeeTeleportSystem.getConfigManager().getMessage("Home.Help"));
					}
				} else player.sendMessage(BungeeTeleportSystem.getConfigManager().getMessage("Home.Help"));
